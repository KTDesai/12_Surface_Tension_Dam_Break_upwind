...
{ 
  ...
  double delta_time = 0.0001, nu_val = 0.001, alpha_u = 0.7, initial_pressure = 0.0, alpha_initial = 0.0, initial_rho = 0.0, initial_mu = 0.0;
  ...
  cavity.set_alpha_scalar_initial_fields(list_of_cells, rho_1, rho_2, nu_1, nu_2, mu_1, mu_2, x_w_max, y_w_max);
   ...
   while(iteration_no <5000)
   {
     ...
     iteration_no = iteration_no +1; 
      ...
      cavity.alpha_convection_discretization(list_of_faces, list_of_cells, list_of_boundaries, alpha_boundary, velocity_boundary);
     ...
     //cavity.velocity_compute_diffusion_matrix_old(list_of_faces, list_of_boundaries, velocity_boundary);
     cavity.velocity_compute_convection_matrix(list_of_faces, list_of_boundaries, velocity_boundary);
     //cavity.velocity_compute_convection_matrix_old(list_of_faces, list_of_boundaries, velocity_boundary);
     cavity.velocity_compute_source_matrix(list_of_cells, list_of_faces);
     ...
     //cavity.velocity_compute_div_u(list_of_cells, total_cells); 
      ...
      // {
      //   //std::cout<<"The face_flux values from the main function are:"<<list_of_faces[i].get_face_flux()<<std::endl;
      // }
